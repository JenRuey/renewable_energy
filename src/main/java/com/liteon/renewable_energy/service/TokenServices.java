...
public class TokenServices {
	...
	public User googlelogin(GoogleLoginRequest req) {
		...
		try {
			InputStream is = new URL("https://oauth2.googleapis.com/tokeninfo?access_token=" + req.getAccess_token())
					.openStream();
		...
		try {
			...
			if (user == null) {
				...
				user = new User(req.getEmail(),
						utils.hashSecurePassword(utils.generateSecurePassword(), utils.getSalt()), new Date());
				user.setId(0);
	...
	public User googlelogin2(GoogleLoginRequest req) {
		try {
			...
			if (isValidToken) {
				...
				if (user == null) {
					SecureUtils utils = new SecureUtils();
					user = new User(req.getEmail(), utils.hashSecurePassword(
							// utils.generateSecurePassword()
							"Abcd123$", utils.getSalt()), new Date());
					return obtainToken(userRepo.save(user));
				} else {
					return obtainToken(user);
				}
